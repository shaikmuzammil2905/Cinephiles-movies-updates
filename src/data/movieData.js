export const heroArticles = [
  {
    id: 'hero-1',
    badge: 'TOP STORY',
    movieTag: 'Kalki 2898 AD',
    actor: 'Prabhas',
    title: 'Kalki 2898 AD Becomes Fastest Indian Film to Cross ₹1100 Crore Worldwide',
    date: 'Aug 15, 2026',
    views: '12.5K Views',
    image: '/kalki.png',
    poster: '/kalki.png',
    summary: 'Nag Ashwin’s epic sci-fi spectacle Kalki 2898 AD starring Rebel Star Prabhas, Amitabh Bachchan, and Kamal Haasan shatters global box office records.',
    content: `Nag Ashwin's visionary sci-fi epic 'Kalki 2898 AD' has made cinematic history by becoming the fastest Indian film to smash past the monumental ₹1100 Crore milestone at the global box office.

Starring Rebel Star Prabhas as Bhairava alongside Indian screen legends Amitabh Bachchan as Ashwatthama and Kamal Haasan as Supreme Yaskin, the futuristic mythological saga has captivated audiences worldwide.

Key Box Office Highlights:
• India Net Collection: ₹645 Crore across all languages.
• Telangana & Andhra Pradesh Gross: ₹285 Crore share.
• North America Gross: $18.5 Million (~₹154 Crore).
• Worldwide Gross: ₹1100+ Crore.`
  },
  {
    id: 'hero-2',
    badge: 'BOX OFFICE RECORD',
    movieTag: 'Pushpa 2: The Rule',
    actor: 'Allu Arjun',
    title: 'Pushpa 2: The Rule Crosses ₹1500 Cr Worldwide, Allu Arjun Creates History',
    date: 'Aug 15, 2026',
    views: '18.9K Views',
    image: '/pushpa2.png',
    poster: '/pushpa2.png',
    summary: 'Icon Star Allu Arjun and director Sukumar shatter all existing box office benchmarks with Pushpa 2: The Rule, dominating mass centers across the globe.',
    content: `Pushpa 2: The Rule has surpassed all expectations, collecting over ₹1500 Crore worldwide. The film has dominated both single screens and multiplexes in the Hindi belt, South India, and international markets.

Sukumar's electrifying direction, Devi Sri Prasad's background score, and Rashmika Mandanna's performance alongside Allu Arjun's iconic act as Pushpa Raj have turned this into a monumental pop culture phenomenon.`
  },
  {
    id: 'hero-3',
    badge: 'EXCLUSIVE UPDATE',
    movieTag: 'SSMB29',
    actor: 'Mahesh Babu',
    title: 'SSMB29: Mahesh Babu & SS Rajamouli Jungle Adventure Shooting Commences in Kenya',
    date: 'Aug 15, 2026',
    views: '24.1K Views',
    image: '/ssmb29.png',
    poster: '/ssmb29.png',
    summary: 'India’s most anticipated globe-trotting action adventure directed by SS Rajamouli officially kicks off principal photography in African wilderness.',
    content: `Superstar Mahesh Babu has landed in Nairobi for the first major schedule of SSMB29. Written by Vijayendra Prasad, the film is backed by a global Hollywood studio partner and features cutting-edge animatronics, live-action stunts, and never-before-seen jungle adventure sequences.`
  }
];

export const trendingNow = [
  {
    id: 1,
    title: 'Sitaare Zameen Par Trailer Out Now, Fans Love It',
    time: '2 hours ago',
    image: '/sitaare.png'
  },
  {
    id: 2,
    title: "Tom Cruise's Mission: Impossible 8 Gets a New Release Date",
    time: '3 hours ago',
    image: '/mi8.png'
  },
  {
    id: 3,
    title: "Pawan Kalyan's They Call Him OG Massive Action Teaser",
    time: '5 hours ago',
    image: '/og.png'
  },
  {
    id: 4,
    title: 'Brad Pitt’s F1 The Movie Official Announcement',
    time: '6 hours ago',
    image: '/f1.png'
  },
  {
    id: 5,
    title: 'Devara Part 1 Digital Release Breaks Streaming Records',
    time: '8 hours ago',
    image: '/devara.png'
  }
];

export const ottPlatforms = [
  { id: 'all', name: 'All OTT', icon: 'Sparkles', color: '#d90429' },
  { id: 'netflix', name: 'Netflix', logo: 'N', color: '#E50914', badgeBg: 'bg-red-600' },
  { id: 'prime', name: 'Prime Video', logo: 'prime video', color: '#00A8E1', badgeBg: 'bg-sky-500' },
  { id: 'jiohotstar', name: 'JioHotstar', logo: '★', color: '#105BEA', badgeBg: 'bg-blue-600' },
  { id: 'sonyliv', name: 'Sony LIV', logo: 'LIV', color: '#FF6F00', badgeBg: 'bg-amber-600' },
  { id: 'zee5', name: 'ZEE5', logo: 'ZEE5', color: '#A000A0', badgeBg: 'bg-purple-600' },
  { id: 'aha', name: 'aha', logo: 'aha', color: '#FF3B00', badgeBg: 'bg-orange-600' },
  { id: 'apple', name: 'Apple TV+', logo: 'tv+', color: '#A3A3A3', badgeBg: 'bg-neutral-700' }
];

export const ottUpdates = [
  {
    id: 'ott-1',
    platform: 'netflix',
    platformName: 'Netflix',
    title: 'Kalki 2898 AD (Hindi)',
    status: 'Streaming Now',
    releaseDate: 'Aug 10, 2026',
    poster: '/kalki.png',
    quality: '4K Ultra HD • Dolby Atmos',
    language: 'Hindi, English Subtitles',
    description: 'The futuristic mythological magnum opus featuring Prabhas and Amitabh Bachchan is now streaming in 4K HDR.'
  },
  {
    id: 'ott-2',
    platform: 'prime',
    platformName: 'Prime Video',
    title: 'Pushpa 2: The Rule',
    status: 'Streaming Now',
    releaseDate: 'Aug 14, 2026',
    poster: '/pushpa2.png',
    quality: '4K UHD • HDR10+',
    language: 'Telugu, Tamil, Malayalam, Kannada',
    description: 'Icon Star Allu Arjun’s record breaking mass action saga hits digital screens worldwide with bonus extended cuts.'
  },
  {
    id: 'ott-3',
    platform: 'jiohotstar',
    platformName: 'JioHotstar',
    title: 'They Call Him OG',
    status: 'Coming Soon',
    releaseDate: 'September 2026',
    poster: '/og.png',
    quality: '4K Dolby Vision',
    language: 'Telugu, Hindi, Tamil',
    description: 'Sujeeth’s high-voltage gangster action thriller starring Powerstar Pawan Kalyan arrives on JioHotstar.'
  },
  {
    id: 'ott-4',
    platform: 'sonyliv',
    platformName: 'Sony LIV',
    title: 'Devara: Part 1',
    status: 'Streaming Now',
    releaseDate: 'Aug 02, 2026',
    poster: '/devara.png',
    quality: 'HD 1080p',
    language: 'Telugu, Hindi, Tamil, Kannada',
    description: 'Man of Masses NTR Jr commands the high seas in Koratala Siva’s rustic action drama with Saif Ali Khan.'
  },
  {
    id: 'ott-5',
    platform: 'aha',
    platformName: 'aha',
    title: 'Thandel',
    status: 'Streaming Now',
    releaseDate: 'Aug 12, 2026',
    poster: '/thandel.png',
    quality: '4K Ultra HD',
    language: 'Telugu Exclusive',
    description: 'Naga Chaitanya and Sai Pallavi star in an emotional fisherman romance survival story inspired by real events.'
  },
  {
    id: 'ott-6',
    platform: 'zee5',
    platformName: 'ZEE5',
    title: 'HanuMan Extended Version',
    status: 'Streaming Now',
    releaseDate: 'Aug 05, 2026',
    poster: '/hanuman.png',
    quality: '4K Ultra HD',
    language: 'Telugu, Hindi, Tamil, Kannada, Malayalam',
    description: 'Prasanth Varma’s superhero blockbuster with unseen deleted scenes and VFX enhancement.'
  }
];

export const movieNews = {
  featured: {
    id: 'news-feat-1',
    category: 'EXCLUSIVE NEWS',
    title: "Allu Arjun's Pushpa 3 Officially Announced",
    time: '2 hours ago',
    views: '8.2K views',
    image: '/pushpa2.png',
    summary: 'Producers Mythri Movie Makers officially confirm Pushpa 3: The Rampage. Sukumar and Allu Arjun to re-team for final trilogy installment.',
    content: `Following the unparalleled global success of Pushpa 2: The Rule, producers Mythri Movie Makers have officially greenlit 'Pushpa 3: The Rampage'. Director Sukumar has completed the story board for the grand finale. Shooting is scheduled to begin in early 2027.`
  },
  list: [
    {
      id: 'news-2',
      title: 'SSMB29: Mahesh Babu Starts Special Jungle Training in Nairobi',
      time: '3 hours ago',
      views: '15.6K views',
      image: '/ssmb29.png',
      summary: 'Director SS Rajamouli subjects Mahesh Babu to rigorous survival martial arts training for the high-budget African adventure film.'
    },
    {
      id: 'news-3',
      title: 'They Call Him OG Climax Shoot Wraps Up in Mangalore Port',
      time: '4 hours ago',
      views: '12.1K views',
      image: '/og.png',
      summary: 'Powerstar Pawan Kalyan and Emraan Hashmi shoot high-octane vintage weapons sequence directed by Sujeeth.'
    },
    {
      id: 'news-4',
      title: 'Devara Part 2 Script Finalized, Shoot Starts October',
      time: '6 hours ago',
      views: '11.4K views',
      image: '/devara.png',
      summary: 'Koratala Siva confirms NTR Jr and Bobby Deol face-off sequence will be shot on a grand scale in Goa coast.'
    }
  ]
};

export const boxOfficeSummary = [
  { rank: 1, movie: 'Pushpa 2: The Rule', indiaNet: '₹820 Cr', worldwide: '₹1500 Cr', verdict: 'All-Time Blockbuster', poster: '/pushpa2.png' },
  { rank: 2, movie: 'Kalki 2898 AD', indiaNet: '₹645 Cr', worldwide: '₹1100 Cr', verdict: 'Blockbuster', poster: '/kalki.png' },
  { rank: 3, movie: 'Devara: Part 1', indiaNet: '₹310 Cr', worldwide: '₹520 Cr', verdict: 'Blockbuster', poster: '/devara.png' },
  { rank: 4, movie: 'HanuMan', indiaNet: '₹215 Cr', worldwide: '₹350 Cr', verdict: 'Super Hit', poster: '/hanuman.png' },
  { rank: 5, movie: 'They Call Him OG', indiaNet: '₹180 Cr', worldwide: '₹290 Cr', verdict: 'Expected Industry Hit', poster: '/og.png' }
];

export const tollywoodSecondWeekRecords = [
  { rank: 1, movie: 'Baahubali 2: The Conclusion', hero: 'Prabhas', director: 'SS Rajamouli', tgapSecondWeekShare: '₹68.40 Cr', indiaNetSecondWeek: '₹214.50 Cr', totalWorldwide: '₹1810 Cr', year: '2017', verdict: 'All-Time Industry Hit' },
  { rank: 2, movie: 'Kalki 2898 AD', hero: 'Prabhas / Amitabh Bachchan', director: 'Nag Ashwin', tgapSecondWeekShare: '₹48.20 Cr', indiaNetSecondWeek: '₹152.00 Cr', totalWorldwide: '₹1100+ Cr', year: '2024', verdict: 'Blockbuster' },
  { rank: 3, movie: 'RRR', hero: 'NTR Jr / Ram Charan', director: 'SS Rajamouli', tgapSecondWeekShare: '₹44.50 Cr', indiaNetSecondWeek: '₹138.20 Cr', totalWorldwide: '₹1387 Cr', year: '2022', verdict: 'Global Blockbuster' },
  { rank: 4, movie: 'Pushpa 2: The Rule', hero: 'Allu Arjun', director: 'Sukumar', tgapSecondWeekShare: '₹42.80 Cr', indiaNetSecondWeek: '₹145.00 Cr', totalWorldwide: '₹1500+ Cr', year: '2024', verdict: 'All-Time Blockbuster' },
  { rank: 5, movie: 'HanuMan', hero: 'Teja Sajja', director: 'Prasanth Varma', tgapSecondWeekShare: '₹32.10 Cr', indiaNetSecondWeek: '₹94.50 Cr', totalWorldwide: '₹350 Cr', year: '2024', verdict: 'Sensation Mega Blockbuster' }
];

export const latestReviews = [
  {
    id: 'rev-1',
    title: 'Pushpa 2: The Rule',
    rating: '4.5',
    director: 'Sukumar',
    cast: 'Allu Arjun, Rashmika Mandanna, Fahadh Faasil',
    poster: '/pushpa2.png',
    summary: 'Icon Star Allu Arjun delivers a career-best performance in Sukumar’s explosive mass saga.',
    verdict: 'ALL-TIME MASS BLOCKBUSTER',
    content: `Pushpa 2: The Rule is a masterclass in commercial Indian cinema. Allu Arjun's screen presence, particularly during the Jaathara sequence and high-octane action set pieces, elevates the film into an unmissable spectacle.`
  },
  {
    id: 'rev-2',
    title: 'Kalki 2898 AD',
    rating: '4.0',
    director: 'Nag Ashwin',
    cast: 'Prabhas, Amitabh Bachchan, Kamal Haasan, Deepika Padukone',
    poster: '/kalki.png',
    summary: 'A visual spectacle of international standards combined with Indian mythology.',
    verdict: 'MUST WATCH CINEMATIC MASTERPIECE',
    content: `Nag Ashwin delivers an extraordinary cinematic experience that seamlessly weaves Indian mythology with futuristic world-building. Prabhas shines in a character filled with charisma and heroism, while Amitabh Bachchan steals the show as Ashwatthama.`
  },
  {
    id: 'rev-3',
    title: 'Devara: Part 1',
    rating: '4.0',
    director: 'Koratala Siva',
    cast: 'NTR Jr, Saif Ali Khan, Janhvi Kapoor',
    poster: '/devara.png',
    summary: 'Man of Masses NTR Jr commands the high seas in Koratala Siva’s visceral action drama.',
    verdict: 'HIGH OCTANE COASTAL ACTION DRAMA',
    content: `NTR Jr delivers an outstanding dual-shaded act in Devara. Anirudh Ravichander’s energetic background score and Koratala Siva’s intense coastal action set pieces create a memorable experience.`
  },
  {
    id: 'rev-4',
    title: 'Thandel',
    rating: '4.0',
    director: 'Chandoo Mondeti',
    cast: 'Naga Chaitanya, Sai Pallavi',
    poster: '/thandel.png',
    summary: 'Heart-wrenching romantic survival drama with soulful music and realistic sea action.',
    verdict: 'EMOTIONAL & INSPIRING ROMANTIC DRAMA',
    content: `Naga Chaitanya and Sai Pallavi deliver deeply moving performances in Thandel. The coastal visuals, authentic dialect, and gripping survival sequences make this a winner.`
  }
];

export const upcomingReleases = [
  {
    id: 'up-1',
    title: 'Mission: Impossible - The Final Reckoning',
    releaseDate: 'August 23, 2026',
    days: 8,
    hrs: 12,
    mins: 45,
    poster: '/mi8.png'
  },
  {
    id: 'up-2',
    title: 'Lilo & Stitch (Live Action)',
    releaseDate: 'August 23, 2026',
    days: 8,
    hrs: 12,
    mins: 45,
    poster: '/lilo.png'
  },
  {
    id: 'up-3',
    title: 'SSMB29 (Mahesh Babu - SS Rajamouli)',
    releaseDate: 'September 15, 2026',
    days: 30,
    hrs: 10,
    mins: 30,
    poster: '/ssmb29.png'
  },
  {
    id: 'up-4',
    title: 'They Call Him OG (Pawan Kalyan)',
    releaseDate: 'September 27, 2026',
    days: 42,
    hrs: 8,
    mins: 15,
    poster: '/og.png'
  }
];

export const latestTrailers = [
  {
    id: 'tr-1',
    title: 'Sitaare Zameen Par Official Trailer (Aamir Khan)',
    duration: '2:45',
    time: '2 hours ago',
    views: '12.4M views',
    youtubeId: 'dQw4w9WgXcQ',
    thumbnail: '/sitaare.png'
  },
  {
    id: 'tr-2',
    title: 'F1 The Movie Official Trailer (Brad Pitt)',
    duration: '2:31',
    time: '4 hours ago',
    views: '18.5M views',
    youtubeId: 'dQw4w9WgXcQ',
    thumbnail: '/f1.png'
  },
  {
    id: 'tr-3',
    title: 'Devara Part 1 Official Trailer (NTR Jr, Saif Ali Khan)',
    duration: '3:05',
    time: '6 hours ago',
    views: '25.8M views',
    youtubeId: 'dQw4w9WgXcQ',
    thumbnail: '/devara.png'
  },
  {
    id: 'tr-4',
    title: 'Thandel Official Glimpse (Naga Chaitanya, Sai Pallavi)',
    duration: '2:19',
    time: '12 hours ago',
    views: '9.1M views',
    youtubeId: 'dQw4w9WgXcQ',
    thumbnail: '/thandel.png'
  },
  {
    id: 'tr-5',
    title: 'HanuMan Official Extended Trailer (Teja Sajja)',
    duration: '2:40',
    time: '1 day ago',
    views: '14.3M views',
    youtubeId: 'dQw4w9WgXcQ',
    thumbnail: '/hanuman.png'
  }
];
